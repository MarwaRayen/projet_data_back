...
def get_data(ind, date_debut, date_fin):
    data = yf.download(
        ind, 
        start=date_debut.strftime(DATE_FORMAT), 
        end=date_fin.strftime(DATE_FORMAT), 
        progress=False,
        auto_adjust=False  # IMPORTANT: Force l'inclusion de 'Adj Close'
    )
    
    # Si le DataFrame a un MultiIndex pour les colonnes, on le flatten
...
def get_rendement_actif_unique(actif: str, date_debut: datetime, date_fin: datetime, montant_initial: int, montant_recurrent: int):
    ...
    if serie_adj_close.empty:
        print(f"Aucune donnée pour {actif} entre {date_debut} et {date_fin}")
        return pd.DataFrame()  # ou gérer autrement
